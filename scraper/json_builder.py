...
def write_to_json():
  ...
  with open(file_path, "w", encoding="utf-8") as file: 
    ...
    for index, obj in enumerate(json_objects):
      ...
      if index != len(json_objects) -1: file.write(",\n")
      else: file.write("\n")  
    file.write( "]")
...
def upsert_json():
  ...
  if condition_choice == 'N': new_obj = build_json(product_id, build_condition())
  elif condition_choice == 'E': new_obj = edit_condition(product_id) 

  # remove old obj
  json_objects[:] = [obj for obj in json_objects if obj.get('id') != new_obj.get('id')]
